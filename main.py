...
def main():
    ...
    agents = { "Supervisor": SupervisorAgent(llm_client, budget_manager, tool_registry), "Researcher": ResearcherAgent(llm_client, budget_manager, tool_registry), "Contrarian": ContrarianAgent(llm_client, budget_manager, tool_registry), "QualityAssessor": QualityAssessorAgent(llm_client, budget_manager), "Fixer": FixerAgent(llm_client, budget_manager), "SanityCheckCritic": SanityCheckCritic(llm_client, budget_manager), "Analyst": AnalystAgent(llm_client, budget_manager, tool_registry), "ReportWriter": ReportWriterAgent(llm_client, budget_manager), "Architect": architect, "Janitor": KnowledgeJanitorAgent(llm_client, budget_manager), "ToolSmith": tool_smith }
    ...
    if args.new_plan_keep_kb and os.path.exists(state_file_path):
        ...
        state_to_run: GraphState = {
            ...
            "knowledge_base": old_state.get("knowledge_base", {}), # <-- СОХРАНЯЕМ БАЗУ ЗНАНИЙ
            "task_queue": [], "completed_tasks": [], "current_task": None,
            "model_assignments": {}, "escalation_count": 0, "error_message": None, "node_outputs": {}
        }
