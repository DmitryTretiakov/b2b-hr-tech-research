...
import json
import os
import traceback  # Импортируем traceback для детальных логов
...
class SemanticIndex:
    ...
    def add_claim(self, claim_id: str, claim_text: str, index_path: str, id_map_path: str):
        """
        Добавляет одно утверждение в индекс. Вызывается в ходе нормальной работы.
        """
    ...
    def _add_claim_internal(self, claim_id: str, claim_text: str):
        ...
        try:
            model_name = self.embedding_model.model
            if not self.budget_manager.can_i_spend(model_name):
                # При перестройке мы не падаем, а просто логируем и пропускаем
