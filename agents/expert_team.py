...
Ты ОБЯЗАН вернуть результат в формате JSON, соответствующем предоставленной схеме. Все утверждения должны иметь статус 'UNVERIFIED'."""
    ...
    def _execute_financial_task(self, task: dict, world_model: WorldModel):
        ...
        prompt = f"""**ТВОЯ РОЛЬ:** Финансовый аналитик в стартапе. Ты прагматичен и консервативен.Ты работаешь ТОЛЬКО с предоставленными данными. Ты не имеешь права ничего додумывать или использовать свои общие знания.
 ...
 **СТРОГИЕ ПРАВИЛА:**
    1.  Ты можешь использовать ТОЛЬКО цифры и факты из предоставленной "БАЗЫ ЗНАНИЙ ДЛЯ АНАЛИЗА".
    2.  Если в базе знаний нет данных (например, о зарплате QA), ты ОБЯЗАН указать это в отчете как "данные отсутствуют" или "требуется дополнительное исследование". НЕ ПРИДУМЫВАЙ ЦИФРЫ.
    3.  Каждое число в твоих таблицах, которое взято из факта, ДОЛЖНО сопровождаться цитатой в формате [Источник: claim_id].
    4.  Твои выводы должны быть прямым следствием анализа предоставленных фактов.
