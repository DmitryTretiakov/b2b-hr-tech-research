...
def main():
    ...
    parser = argparse.ArgumentParser(description="Запуск AI Factory v4.5")
    ...
    print("Инициализация системы 'Динамический Фреймворк v4.5'...")
    ...
    agents = {
        "Supervisor": SupervisorAgent(llm_client, budget_manager), "Architect": architect, "Validator": validator,
        "Janitor": KnowledgeJanitorAgent(llm_client, budget_manager), "FailureAnalyst": failure_analyst,
        ...
        "QualityAssessor": QualityAssessorAgent(llm_client, budget_manager), "Fixer": FixerAgent(llm_client, budget_manager),
        "SanityCheckCritic": SanityCheckCritic(llm_client, budget_manager), "OutlineAgent": OutlineAgent(llm_client, budget_manager),
        "SectionWriterAgent": SectionWriterAgent(llm_client, budget_manager), "ReportWriter": ReportWriterAgent(llm_client, budget_manager),
        "FinancialModelAgent": FinancialModelAgent(llm_client, budget_manager), "ProductManagerAgent": ProductManagerAgent(llm_client, budget_manager),
   ...
   # --- 3. Сборка графа и определение начального состояния ---
    ...
    initial_state = GraphState(
        ...
        knowledge_base={},
        artifacts={},
        data_bus={}, # Инициализируем шину данных
        model_assignments={},
        visited_urls=[],
        escalation_count=0,
        current_task=None,
    ...
    if os.path.exists(STATE_FILE) and not args.new_plan_keep_kb:
        ...
        # Убедимся, что data_bus существует, если мы загружаем старый стейт
        if 'data_bus' not in initial_state:
            initial_state['data_bus'] = {}
        ...
        if args.new_plan_keep_kb and os.path.exists(STATE_FILE):
            ...
            with open(STATE_FILE, "r", encoding="utf-8") as f:
                saved_state = json.load(f)
            # --- ИЗМЕНЕНИЕ НАЧАТО: Явно очищаем data_bus при пересоздании плана ---
            initial_state['knowledge_base'] = saved_state.get('knowledge_base', {})
            initial_state['data_bus'] = {} # Очищаем шину данных
            print("   [Main] <- База Знаний загружена. Шина данных очищена. Генерирую новый план.")
            # --- ИЗМЕНЕНИЕ ОКОНЧЕНО ---
        else:
